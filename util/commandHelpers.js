    ...
    getIdFromMention(mentionStr) {
        const regExp = /\d+/;
        if (!regExp.test(mentionStr)) return false;
        return mentionStr.match(regExp)[0];
    },

    throwUserError(errorMessage, message) {
        throw {
            message,
            name: 'userError',
            errorMessage,
    ...
    getCommandAndArgs (message, prefix = process.env.PREFIX) {
    ...
    createMultiChoiceEmbedFields(options) {
    ...
    async reactInSequenceasy(message, emojis) {
